...
public class PreProcessingHandlerTest {
    ...
    public void before() throws IOException {
        messages = ResourceBundles.getBundle("actions");
    ...
    public void testSupplierRelations9() throws Exception {
        ...
        when(rawRepo.recordExists(anyString(), anyInt())).thenReturn(false);
        when(rawRepo.recordExists(eq("46079922"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("46079922"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(head, MarcXChangeMimeType.MARCXCHANGE));

        PreProcessingHandler preProcessingHandler = new PreProcessingHandler(rawRepo, messages);
    ...
    public void testSupplierRelations11() throws Exception {
        testExample("preprocessing/supplier-relations/test-11-input.marc",
                "preprocessing/supplier-relations/test-11-expected.marc");
    }

    @Test
    public void testSupplierRelations12() throws Exception {
        testExample("preprocessing/supplier-relations/test-12-input.marc",
                "preprocessing/supplier-relations/test-12-expected.marc");
    }
    ...
    public void testPreviousISBN1() throws Exception {
        ...
        when(rawRepo.recordExists(eq("29469237"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("29469237"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(previous, MarcXChangeMimeType.MARCXCHANGE));
    ...
    public void testPreviousISBN2() throws Exception {
        ...
        when(rawRepo.recordExists(eq("52079020"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("52079020"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(previous1, MarcXChangeMimeType.MARCXCHANGE));
        when(rawRepo.recordExists(eq("52106249"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("52106249"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(previous2, MarcXChangeMimeType.MARCXCHANGE));
    ...
    public void testPreviousISBN3_ParentIsHead() throws Exception {
        ...
        when(rawRepo.recordExists(eq("05259282"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("05259282"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(previous, MarcXChangeMimeType.MARCXCHANGE));
        when(rawRepo.recordExists(eq("54948441"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("54948441"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(requestParent, MarcXChangeMimeType.MARCXCHANGE));
    ...
    public void testPreviousISBN3_ParentIsSection() throws Exception {
        final MarcRecord request = AssertActionsUtil.loadRecord("preprocessing/isbn-previous-version/test-3-request.marc");
        final MarcRecord expected = AssertActionsUtil.loadRecord("preprocessing/isbn-previous-version/test-3-request.marc");
        final MarcRecord previous = AssertActionsUtil.loadRecord("preprocessing/isbn-previous-version/test-3-rawrepo-05259282-870970.marc");
    ...
    public void testPreviousISBN6() throws Exception {
        ...
        final MarcRecord expected = AssertActionsUtil.loadRecord("preprocessing/isbn-previous-version/test-6-request.marc");

        when(rawRepo.recordExists(eq("29469237"), eq(870970))).thenReturn(false);
    ...
    public void testPreviousISBN7() throws Exception {
        ...
        when(rawRepo.recordExists(eq("50953033"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("50953033"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(previous, MarcXChangeMimeType.MARCXCHANGE));
        when(rawRepo.recordExists(eq("27364500"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("27364500"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(headVolume, MarcXChangeMimeType.MARCXCHANGE));
        when(rawRepo.recordExists(eq("27430961"), eq(870970))).thenReturn(true);
        when(rawRepo.fetchRecord(eq("27430961"), eq(870970))).thenReturn(AssertActionsUtil.createRawRepoRecord(sectionVolume, MarcXChangeMimeType.MARCXCHANGE));
    ...
    private void testExample(String inputFileName, String expectedFileName, String existingFileName) throws Exception {
        ...
        when(rawRepo.recordExistsMaybeDeleted(anyString(), anyInt())).thenReturn(existingFileName != null);
        when(rawRepo.fetchRecord(anyString(), anyInt())).thenReturn(AssertActionsUtil.createRawRepoRecord(existing, MarcXChangeMimeType.MARCXCHANGE));

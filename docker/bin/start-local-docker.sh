...
then
    ...
    echo "ERROR!"
    echo "${TEST_RUN} must exist in order to run this script, but the file is missing."
    echo "A possible cause for this is that start_dev_docker.sh has not been executed yet."
    exit 1
fi

export SOLR_PORT_NR=$(readOcbProperty solr.port)
DOCKER_FOLDER=${IDEA_ROOT}/docker/update-payara-dev

export HOST_IP="$(getHostIP)"
echo "HOST_IP: $HOST_IP"

cd ${IDEA_ROOT} || fail "Failed to cd into ${IDEA_ROOT}"
...
cd docker/deployments/dev || fail "Failed to cd into docker/deployments/dev"
...
export DEV_RAWREPO_DB_URL=$(readOcbProperty rawrepo.db.url)
export DEV_UPDATE_DB_URL=$(readOcbProperty updateservice.db.url)
export DEV_OPENCAT_BUSINESS_URL=$(readOcbProperty opencat.business.url)
export DEV_HOLDINGS_URL=$(readOcbProperty holdings.url)
...
sed -i -e "/^buildservice.url/s/^.*$/buildservice.url = http:\/\/${HOST_IP}:${UPDATESERVICE_FACADE_PORT_8080}/" "${TEST_RUN}"
sed -i -e "/^updateservice.url/s/^.*$/updateservice.url = http:\/\/${HOST_IP}:${UPDATESERVICE_FACADE_PORT_8080}/" "${TEST_RUN}"

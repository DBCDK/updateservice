...
echo "rawrepo.jdbc.driver = org.postgresql.Driver" > "${TEST_RUN}"
echo "rawrepo.jdbc.conn.url = jdbc:postgresql://${HOST_IP}:${RAWREPO_PORT}/rawrepo" >> "${TEST_RUN}"
echo "rawrepo.jdbc.conn.user = rawrepo" >> "${TEST_RUN}"
echo "rawrepo.jdbc.conn.passwd = thePassword" >> "${TEST_RUN}"
echo "rawrepo.db.url = rawrepo:thePassword@${HOST_IP}:${RAWREPO_PORT}/rawrepo" >> "${TEST_RUN}"

echo "updateservice.db.url = updateservice:thePassword@${HOST_IP}:${UPDATESERVICEDB_PORT}/updateservice" >> "${TEST_RUN}"

echo "solr.port = ${SOLR_PORT}" >> "${TEST_RUN}"
echo "holdings.port = ${HOLDINGS_PORT}" >> "${TEST_RUN}"
export DEV_HOLDINGS_URL="http://localhost:${HOLDINGS_PORT}/api"
echo "holdings.url" = ${DEV_HOLDINGS_URL} >> "${TEST_RUN}"

#Set x.forwarded.for to the ip of devel8. This way it is possible to run the test behind the vpn
echo "request.headers.x.forwarded.for = 172.17.20.165" >> "${TEST_RUN}"

echo "rawrepo.provider.name.dbc = dataio-update" >> "${TEST_RUN}"
echo "rawrepo.provider.name.fbs = opencataloging-update" >> "${TEST_RUN}"
echo "rawrepo.provider.name.ph = fbs-ph-update" >> "${TEST_RUN}"
echo "rawrepo.provider.name.ph.holdings = dataio-ph-holding-update" >> "${TEST_RUN}"
...
echo "rawrepo.record.service.url = http://${HOST_IP}:${RAWREPO_RECORD_SERVICE_PORT}" >> "${TEST_RUN}"
...
echo "opencat.business.url = http://${HOST_IP}:${OPENCAT_BUSINESS_SERVICE_PORT}" >> "${TEST_RUN}"
...
echo "updateservice.url = dummy" >> "${TEST_RUN}"
echo "buildservice.url = dummy" >> "${TEST_RUN}"

"${DOCKER_BIN}/healthcheck-rawrepo-record-service.sh" ${HOST_IP} ${RAWREPO_RECORD_SERVICE_PORT} 220 || die "could not start rawrepo-record-service"
"${DOCKER_BIN}/healthcheck-opencat-business-service.sh" ${HOST_IP} ${OPENCAT_BUSINESS_SERVICE_PORT} 220 || die "could not start opencat-business-service"
